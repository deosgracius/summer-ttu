...
TOOLS = {
    ...
    "elective_catalog": _t("Look up which courses count as approved electives from the departmental catalog/master list, by code, title, or category.", ALL, {"query": {"type": "string"}}, ["query"], elective_catalog),
    ...
    "search_documents": _t("Search the admin-uploaded FILES (spreadsheets, Word documents, slide decks, PDFs — course and electives lists, department handbook, policies, syllabi, FAQs) for passages relevant to a question. Use it for policies, procedures and deadlines, AND as the fallback for any campus question the structured tools come up empty on: the admin often uploads the term's course/electives spreadsheet, so a class, section, time, room or instructor that find_course and course_search don't have may well be in an uploaded file. ALWAYS try this before telling someone the information isn't available. Returns the most relevant passages each WITH A CITATION (document title + section); answer ONLY from those passages and cite them, and if nothing relevant comes back, say so.", ALL, {"query": {"type": "string"}}, ["query"], search_documents),
}
